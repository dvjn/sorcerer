...
func (s *FileSystemStorage) blobDir(name string) string {
	return filepath.Join(s.root, blobsBaseDir, name)
}

func (s *FileSystemStorage) blobPath(name, digest string) string {
	return filepath.Join(s.blobDir(name), digest)
}

func (s *FileSystemStorage) HasBlob(name, digest string) (bool, int64, error) {
...
func (s *FileSystemStorage) GetBlob(name, digest string) (io.ReadCloser, int64, error) {
	path := s.blobPath(name, digest)
...
func (s *FileSystemStorage) PutBlob(name, digest string, content io.Reader) error {
	dir := s.blobDir(name)
...
func (s *FileSystemStorage) DeleteBlob(name, digest string) error {
	path := s.blobPath(name, digest)
...
func (s *FileSystemStorage) MountBlob(fromName, toName, digest string) error {
